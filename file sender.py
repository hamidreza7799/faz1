...
print ('\033[46m\033[34m\033[1mBienvenido al FileSender v.0.02 hecho en Python. Este programapermite enviar archivos a traves de tu maquina\033[0m')
...
def seleccion_path():
	...
	PATH = input('\033[34m\033[1mSelect the Path (./ by default)').strip('n')
	...
	acepta_path = input('\033[34m\033[1mSi o No (S/N)').lower().strip(' ')
...
fileSelected = int(input(ANSI_BLUE + 'Select a file with the number').strip(' ').lower()) 

print (PATH + filesDir(PATH)[fileSelected-1])

filepath = PATH + filesDir(PATH)[fileSelected-1]

conn.send(b'filepath')
