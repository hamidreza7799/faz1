...
class back:
    def __init__(self):
        ...
        self.button=ttk.Button(root,image=self.image,command=self.back_f).grid(row=0,padx=5)
...
forward_button=ttk.Button(root,image=forward_img).grid(row=0,column=1,sticky="w",padx=5)
forward_label=Label(root,text="forward",fg="red").grid(row=2,column=1,sticky="w")
#append
class append:
    def __init__(self):
        self.image=PhotoImage(file="add.png")
        self.button=ttk.Button(root,image=self.image,command=self.append_f).grid(row=0,column=2,sticky="e",padx=5)
...
class search:
    def __init__(self):
        ...
        self.button=ttk.Button(root,image=self.img,command=self.search_f).grid(row=0,column=4,sticky='e')
        self.entry=ttk.Entry(root)
...
class adress_bar:
    def __init__(self):
        ...
        self.label.grid(row=0,column=8,padx=300)
    def creat(self):
        text=os.getcwd()
...
chatentry=ttk.Entry(root)
chatphoto=PhotoImage(file='chat.png')

chatentry.grid(row=0,column=7,sticky='e')
chatentry.insert(10,'IP ADRESS')
        
print(type(chatentry.get()))
print(socket.gethostbyname(socket.gethostname()))
chatlistbox=Listbox(root,selectforeground='red'
                                     ,highlightbackground='red',highlightcolor='black',selectborderwidth=5,
                                     selectbackground='black',selectmode='SINGLE')

chatlistbox.grid(row=2,rowspan=6,column=7,ipady=50,sticky="ne")


class client:
    def __init__(self,adress):
        self.socket=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        self.socket.connect_ex((adress,5000))
    ...
    def run(self):
        while True:
            ...
            if data==b'salam':
                answer = messagebox.askokcancel("Question","Do you want to open this file?",parent=root)
                if answer:
                    self.socket.send('1')
                else:
                    self.socket.send('0')
                ...
                print(data)
                chatlistbox.insert(END,data)
                if not data:
                    self.socket.close()
                
    def senM(self):
        self.socket.send(bytes(chatentry.get(),'UTF-8'))


           
#client1=client('www.google.com')
...
button=ttk.Button(root,image=chatphoto,command=senM).grid(row=0,column=7,padx=150)
...
def D():
    ...
    s.creat_files()
    adress.creat()
    s.listbox.selection_set(END)
    #print(s.listbox.curselection())
...
class scrollbar:
    def __init__(self):
        ...
        self.listbox=Listbox(self.window,yscrollcommand=self.scrollbar.set,selectforeground='red'
                             ,highlightbackground='red',highlightcolor='black',selectborderwidth=5,
                             selectbackground='black',selectmode='SINGLE')
        self.listbox.bind("<<ListboxSelect>>", self.open)
        self.listbox.grid(row=3,rowspan=8,column=4,ipady=50,sticky="ne")
        self.current_path=os.getcwd()
        self.current_path_list=os.listdir(os.getcwd())
        #creat_files()
    def open(self,event):
        answer=event.widget.get(event.widget.curselection())
        #answer = simpledialog.askstring("Input", "please enter your file's name",parent=root)
        try:
